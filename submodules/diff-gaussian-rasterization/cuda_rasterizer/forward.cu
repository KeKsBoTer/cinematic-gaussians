...
__device__ glm::vec3 computeColorFromSH(int idx, int deg, int max_coeffs, const glm::vec3* means, glm::vec3 campos, const float* shs, bool* clamped,bool clamp_color)
{
	...
	bool clamp_color)
{
	...
	{
		glm::vec3 result = computeColorFromSH(idx, D, M, (glm::vec3*)orig_points, *cam_pos, shs, clamped,clamp_color);
	...
	float* __restrict__ out_color)
{
	...
	if (inside)
	{
		...
		out_color[CHANNELS * H * W + pix_id] = 1.0f - T;
	}
}
