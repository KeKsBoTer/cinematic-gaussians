...
def extract_rot_scale(cov: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
    ...
    try:
        S, R = torch.linalg.eigh(cov, UPLO="U")
    except:
        # somethimes it just fails on the GPU...
        print("Warning: torch.linalg.eigh failed on the GPU, falling back to CPU")
        S, R = torch.linalg.eigh(cov.cpu(), UPLO="U")
        S = S.to(cov.device)
        R = R.to(cov.device)
